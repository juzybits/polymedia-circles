...
export class AuthenticatorState {
  ...
  static fromBcs(
    ...
    return AuthenticatorState.fromFields(
      bcs.de([AuthenticatorState.$typeName], data, encoding),
    );
  }
  ...
  static fromSuiParsedData(content: SuiParsedData) {
    ...
    if (!isAuthenticatorState(content.type)) {
      throw new Error(
        ...
        } is not a AuthenticatorState object`,
      );
    }
...
export class AuthenticatorStateInner {
  ...
  static fromFields(fields: Record<string, any>): AuthenticatorStateInner {
    return new AuthenticatorStateInner({
      version: BigInt(fields.version),
      activeJwks: fields.active_jwks.map((item: any) =>
        ActiveJwk.fromFields(item),
      ),
    });
  }
  ...
  static fromFieldsWithTypes(item: FieldsWithTypes): AuthenticatorStateInner {
    ...
    return new AuthenticatorStateInner({
      version: BigInt(item.fields.version),
      activeJwks: item.fields.active_jwks.map((item: any) =>
  ...
  static fromFields(fields: Record<string, any>): JWK {
    return new JWK({
      kty: new TextDecoder()
        ...
        .toString(),
      e: new TextDecoder()
        .decode(Uint8Array.from(String.fromFields(fields.e).bytes))
        .toString(),
      n: new TextDecoder()
        .decode(Uint8Array.from(String.fromFields(fields.n).bytes))
        .toString(),
      alg: new TextDecoder()
