...
export interface MintArtworkViaWhitelistArgs {
  collection: ObjectArg;
  whitelist: ObjectArg;
}

export function mintArtworkViaWhitelist(
  txb: TransactionBlock,
  args: MintArtworkViaWhitelistArgs,
) {
  return txb.moveCall({
    target: `${PUBLISHED_AT}::controller::mint_artwork_via_whitelist`,
    arguments: [obj(txb, args.collection), obj(txb, args.whitelist)],
  });
}
